<template>
    <form @submit.prevent="submit()">
        <div class="grid md:grid-cols-2 mt-2 gap-3">
            ...
            <floating-dropdown v-if="!is_create" :form="el_form" name="type" :required="1"
                :options="formData.types" :label="$t('base.type')" />
         
            <floating-input  v-if="!is_create" :form="el_form"  name="phone" />
...
const el_form = useForm({
    ...
    type: el_row?.type ?? null,
    password:  null,
    is_active: is_create ? true : el_row?.is_active,
})
const submit = () => {
    el_form.post(is_create ? route('hospitalityproviders.store') : route('hospitalityproviders.update', el_form.id), {
        onSuccess: () => {
