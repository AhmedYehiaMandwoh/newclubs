<template>
    <form @submit.prevent="submit()">
        <div class="grid md:grid-cols-2 mt-2 gap-3">
            <div class="col-span-full w-[150px]" id="mainImage">
                <AvatarInput :form="el_form" :old-image-preview="el_form.avatar_url" name="avatar" />
            </div>
            <floating-input :form="el_form" required name="name" />
            <floating-input :form="el_form" required type="email" name="email" />
            <floating-input :form="el_form" required type="phone" name="phone" />
...
const submit = () => {
    el_form.post(is_create ? route('clients.store') : route('clients.update', el_form.id), {
        onSuccess: () => {
